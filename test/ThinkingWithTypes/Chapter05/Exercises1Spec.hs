module ThinkingWithTypes.Chapter05.Exercises1Spec where

import           ThinkingWithTypes.Chapter05.Exercises1()
import           ThinkingWithTypes.Chapter05.Examples1
import           Test.Hspec

testOrdForHList :: Spec
testOrdForHList =
    context "Exercise 5.3-i - `Ord` for `HList`" $ do
...
testShowForHList =
    context "Exercise 5.3-ii - `Show` for `HList`" $ do
