...
data Button = Button Digit [Char] deriving (Eq, Show)

data DaPhone = DaPhone [Button] deriving (Eq, Show)

standardPhone :: DaPhone
standardPhone = DaPhone [Button '1' "1",
                         Button '2' "abc2",
                         Button '3' "def3",
                         Button '4' "ghi4",
                         Button '5' "jkl5",
...
findChar c (Button d cs) = 
    case maybePresses of
             Nothing -> Nothing
             Just x  -> Just (d, x + 1)
    where maybePresses = findIndex (\x -> x == c) cs
...
mostPopularLetter = 
    fst . (maximumBy compareSnd) . frequencies
    where compareSnd (_, ca) (_, cb) = compare ca cb
